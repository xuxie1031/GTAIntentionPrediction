    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in dataloader_train:
            ...
            for num in num2input_dict.keys():
                ...
                one_hots_pred = data_feeder_onehots_pred(pred_sentence, num)

                if args.use_cuda:
                    inputs = inputs.to(dev)
                    batch_pred_data = batch_pred_data.to(dev)
                    As = As.to(dev)
                    one_hots_obs = one_hots_obs.to(dev)
                    one_hots_pred = one_hots_pred.to(dev)

                pred_outs = stgcn_gep(inputs, As, one_hots_obs, one_hots_pred)
...
def main():
    ...
    parser.add_argument('--grammar_file', type=str, default='tmp/NGSIM/grammar/NGSIM_thor.pcfg')
    parser.add_argument('--grammar_prior', type=str, default='tmp/NGSIM/NGSIM_thor_duration_prior.json')
